...
import sqlalchemy
from sqlalchemy import create_engine, String, Integer, SmallInteger, BigInteger, Date, DateTime, Boolean, Numeric, Table, JSON, Binary, BINARY
from sqlalchemy.dialects.postgresql import JSONB, BOOLEAN, NUMERIC, VARCHAR, TEXT, TIMESTAMP, BIGINT, SMALLINT, INTEGER, BYTEA
            ...
            if isinstance(column, str):
                ...
                if isinstance(column.type, (String, VARCHAR, TEXT)):
                    ...
                    if column.autoincrement == True:
                        ...
                        column_type = 'bigint'
                elif isinstance(column.type, (SmallInteger, SMALLINT)) or str(column.type)=='SMALLINT':
                    if column.autoincrement == True:
...
class Postgres:
    ...
    @staticmethod
    def Table(t):
        #log.debug(f'Postgres.Table({t}, {type(t)})')
        table = None
        if isinstance(t, str):
            table_name = t.lower()
            table = TABLES.get(table_name) 
            #log.debug(f'{table_name}={table}, {TABLES}')
            if not table:
                table = PostgresTable(t)
                TABLES[table.table_name] = table
            #log.debug(f'{TABLES}')
           
        elif isinstance(t, sqlalchemy.Table):
            #log.debug(f'{TABLES}')
            table = PostgresTable(t)
            TABLES[table.table_name] = table
            #log.debug(f'{TABLES}')
        else:
            raise Exception(f'Недопустимый тип таблицы {type(t)}')
